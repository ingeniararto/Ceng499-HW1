...
def train_loss_mean_calculator(train_outputs, train_labels):
    array = torch.from_numpy(np.zeros((len(train_outputs),1), dtype=np.float32))
    for i in range(len(train_outputs)):
        for j in range(3):
            array[i] += torch.log(train_outputs[i][j])*train_labels[i][j]
    mean = mean_calc(array)
    return mean
...
def prediction_result(outputs):
    prediction = torch.from_numpy(np.zeros((len(outputs),3), dtype=np.float32))
    
    for n in range(len(outputs)):
        index = return_max_index(outputs[n])
        for i in range(3):
            prediction[n][i] = 1 if index==i else 0
    return prediction

def accuracy_calculate(outputs, training_label):
    sum = 0
    prediction = prediction_result(outputs)
    for n in range(len(prediction)):
        sum += prediction[n][0]*training_label[n][0] + prediction[n][1]*training_label[n][1] + prediction[n][2]*training_label[n][2]
...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    outputs = torch.from_numpy(np.zeros((len(input_data),3), dtype=np.float32))
    for n in range(len(input_data)):
        hidden_layer = torch.from_numpy(np.zeros((1,16), dtype=np.float32))
        hidden_layer = (torch.matmul(input_data[n],w1) + b1).sigmoid()


        outputs[n] = (torch.matmul(hidden_layer,w2) + b2)
        outputs[n]= outputs[n].softmax(0)
...
for iteration in range(1, ITERATION+1):
    ...
    train_mean_crossentropy_loss = train_loss_mean_calculator(train_predictions, train_label)
    ...
    with torch.no_grad():
        ...
        train_accuracy = accuracy_calculate(train_predictions, train_label)
        ...
        validation_mean_crossentropy_loss =  train_loss_mean_calculator(validation_predictions, validation_label)
        ...
        validation_accuracy = accuracy_calculate(validation_predictions, validation_label)
...
with torch.no_grad():
    ...
    test_accuracy = accuracy_calculate(test_predictions, test_label)
